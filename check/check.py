def check(king, queen):
    ...
    king_letter = king[0]
    queen_letter = queen[0]
    king_num = king[1]
    queen_num = queen[1]

    if (king_letter == queen_letter) or (king_num == queen_num):
        return True
    elif abs(alphabet.index(king_letter) - alphabet.index(queen_letter)) == abs(int(king_num) - int(queen_num)):
        return True
    else:
        return False
