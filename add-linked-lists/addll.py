...
def add_linked_lists(l1, l2):
    ...
    return Node(makeLLstr(l1) + makeLLstr(l2))


def makeLLstr(lst):
    '''Take in head node of linked list and return reversed int'''
    add = lst
    num_to_add = str(add.data)
    if add.next:
        while add.next:
            num_to_add += str(add.next.data)
            add = add.next
    else:
        num_to_add = str(add.data)

    return int(reverse(num_to_add))
